...
class CRLoss(nn.Module):
    ...
    def get_triplets(self, similarity, labels, auto_margin_flag, margin):
        ...
        for idx, label in enumerate(labels):  # same class calculate together
            if margin[idx] >= 0.16 or auto_margin_flag is False:
                negative = np.where(labels != label)[0] #bs-1

                ap_sim = similarity[idx, idx] # 1
    ...
    def forward(self, img, txt, txt_cr, labels, auto_margin_flag):
        ...
        similarity, similarity_match = calculate_similarity(img, txt) #(bs,bs) ,bs
        similarity_cr, similarity_cr_match = calculate_similarity(img, txt_cr) #(bs,bs) ,bs
        margin_cr = calculate_margin_cr(similarity_cr_match, similarity_match, auto_margin_flag, self.margin) #bs
